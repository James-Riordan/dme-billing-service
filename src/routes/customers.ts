...
router.post('/', async (req, res) => {
  try {
    ...
    const lagoError = await getLagoError<typeof lagoClient.customers.createCustomer>(error);
...
router.get('/', async (req, res) => {
  try {
    ...
    const lagoError = await getLagoError<typeof lagoClient.customers.findAllCustomers>(error);
    res.status(500).json({ error: lagoError });
  }
});
...
router.get('/:customerId', async (req, res) => {
  try {
    ...
    const lagoError = await getLagoError<typeof lagoClient.customers.findCustomer>(error);
    res.status(500).json({ error: lagoError });
  }
});
...
router.delete('/:customerId', async (req, res) => {
  try {
    ...
    const lagoError = await getLagoError<typeof lagoClient.customers.destroyCustomer>(error);
    res.status(500).json({ error: lagoError });
  }
});
